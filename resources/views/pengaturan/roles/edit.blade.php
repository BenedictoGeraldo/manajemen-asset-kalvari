            ...
            <div class="mb-6">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border {{ $errors->has('description') ? 'border-red-500' : 'border-gray-300' }} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                                                ...
                                                @foreach($perms as $permission)
                                                    <div class="flex items-start">
                                                        <div class="flex items-center h-5">
                                                            <input id="perm-{{ $permission->id }}" name="permissions[]" type="checkbox" value="{{ $permission->id }}" data-name="{{ $permission->name }}"
                                                                   {{ (is_array(old('permissions')) ? in_array($permission->id, old('permissions')) : in_array($permission->id, $rolePermissions)) ? 'checked' : '' }}
                                                                   class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded transition-colors duration-200 mt-0.5">
                                                        </div>
                                                        <div class="ml-3 text-sm">
                                                            <label for="perm-{{ $permission->id }}" class="font-medium text-gray-700 flex flex-col cursor-pointer select-none">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    groups.forEach(group => {
        ...
        // Temukan checkbox '.view' utama di grup ini
        checkboxes.forEach(cb => {
            if (cb.dataset.name && cb.dataset.name.endsWith('.view')) {
                // Pastikan ini adalah parent view
                viewCheckbox = cb;
            }
        });

        if (viewCheckbox) {
            // Jika view di-uncheck, uncheck semua aksi lainnya
            ...
            // Jika aksi apa pun di-check, view harus otomatis ter-check
            checkboxes.forEach(cb => {
                if (cb !== viewCheckbox) {
                    cb.addEventListener('change', function() {
                        if (this.checked) {
                            ...
                            // Visual feedback sedikit
                            const viewLabel = viewCheckbox.closest('.flex').querySelector('label span');
                            viewLabel.classList.add('text-blue-600');
                            setTimeout(() => viewLabel.classList.remove('text-blue-600'), 500);
                        }
                    });
                }
            });
